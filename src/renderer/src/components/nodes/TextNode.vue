  ...
  <div class="text-node-wrapper" @mouseenter="showHandleMenu = true" @mouseleave="showHandleMenu = false">
    ...
    <div
      ...
      <div class="p-3">
        ...
        <!-- Model selector | 润色模型选择（参考文生图）-->
        <div class="flex items-center justify-between mt-2">
          <span class="text-xs text-[var(--text-secondary)]">润色模型</span>
          <n-dropdown :options="modelOptions" key-field="key" label-field="label" trigger="click" @select="handleModelSelect">
...
// Watch for external model changes | 监听外部模型变化
...
onMounted(() => {
  // 校验润色模型是否可用，否则回退默认 | Validate polish model
...
const handlePolish = async () => {
  ...
  try {
    // Call chat API to polish the prompt | 调用 AI 润色提示词（按次传入选中的模型）
    clearPolish()
